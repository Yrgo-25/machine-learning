...
{
    // Returnera true om input-storleken är samma som antalet vikter vi har i lagret.
...
{
    // Returnera true om referens-storleken är samma som antalet noder i lagret.
...
{
    // Returnera true om antalet vikter i nästa lager är lika med antalet noder i detta lager.
...
{
    // Returnera true om antalet inputs är lika med antalet vikter i detta lager, samt att
    // lärhastigheten ligger i intervallet (0.0, 1.0).
    const auto match   = input.size() == weightCount();
    const auto lrValid = ((0.0 < learningRate) && (1.0 > learningRate));
    return match && lrValid;
}
} // namespace ml::dense_layer
